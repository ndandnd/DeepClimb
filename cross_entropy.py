...
V = np.random.normal(0,var,size = (17,num_hidden)) # hidden to output
...
# %%
pd.DataFrame(W).to_csv("tuned_matrices/ce2017W.csv", header = None, index = None)
pd.DataFrame(V).to_csv("tuned_matrices/ce2017V.csv", header = None, index = None)
...
ax.set_xlabel('Epoch')
ax.set_ylabel('Loss')
image_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figures\\loss_CE.png')
fig.show()
